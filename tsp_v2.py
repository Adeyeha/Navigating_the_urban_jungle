...
def draw_houses(image,no_of_houses,valley_polygons,hill_polygons):
    # Draw houses
    house_polygons = []
...
def draw_buildings(image,valley_polygons,hill_polygons,house_polygons):
...
def draw_tsp_route(image,tsp,center_points,rectilinear = True):
    ...
    for i in range(len(tsp) - 1):
        ...
        node2 = tsp[i + 1]
        if rectilinear:
            draw_rectilinear_line(image, center_points[node1], center_points[node2], 255, 3)
        else:
            cv2.line(image, center_points[node1], center_points[node2], 255, 3)
    return image
...
def draw_mst_route(image, mst, center_points, rectilinear = True):
    for i, j in mst.edges():
        if rectilinear:
            draw_rectilinear_line(image, center_points[i], center_points[j], 255, 3)
        else:
            cv2.line(image, center_points[i], center_points[j], 255, 3)
    return image

# Create Graph and Image

def create_graph(rectilinear = True):
    ...
    # Draw houses
    image,house_polygons = draw_houses(image,no_of_houses,valley_polygons,hill_polygons)

    # Draw buildings with labels
    image,label_polygons,building_polygon = draw_buildings(image,valley_polygons,hill_polygons,house_polygons)
...
def main(rectilinear = True):
    # Create Complete Grpah
    G, image_with_graph_weights, image ,center_points = create_graph(rectilinear)
    ...
    cv2.imshow(f"MST - {mst_length}", mst_image)
    cv2.imshow(f"2_OPT - {two_opt_length}", two_opt_tsp_image)
    cv2.imshow(f"NN - {nnt_length}", nn_tsp_image)
    if no_of_houses <= 6:
        cv2.imshow(f"Brute Force - {optimal_length}", brute_force_tsp_image)

    cv2.waitKey(0)
    cv2.destroyAllWindows()

if __name__ == "__main__":
    main(rectilinear=True)